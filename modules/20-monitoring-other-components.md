    ...
    Identifier les composants matériels périphériques : ILOM, PDU, switches, alimentation, ventilation et capteurs. Le chapitre vise une compréhension opérationnelle et théorique : l’étudiant doit pouvoir expliquer le mécanisme, reconnaître les composants impliqués, lire les principales vues ou commandes et résoudre un cas d’école sans modifier l’environnement.

    ## 2. Pourquoi ce sujet est important

    Exadata dépend aussi de composants non visibles dans SQL. Les alertes matérielles doivent être prises au sérieux car elles peuvent annoncer une panne physique.
    ...
    |---|---|---|
    | **ILOM** | Interface de gestion matérielle out-of-band des serveurs Oracle. | Elle expose journaux matériels, capteurs et état alimentation. |
| **PDU** | Unité de distribution électrique du rack. | Une alerte PDU peut indiquer un risque sur l’alimentation. |
| **Capteur matériel** | Mesure température, ventilateur, tension ou alimentation. | Une température élevée peut précéder un throttling ou une panne. |

    Ces concepts doivent être étudiés ensemble. Par exemple, **ILOM** n’a pas la même signification isolément que dans une architecture RAC, ASM et storage cells. La compréhension vient de la relation entre objet Oracle, ressource Exadata et workload applicatif.
...
tfactl print status
    ...
    |---|---|
    | ILOM | Cette information indique comment le mécanisme ILOM se comporte dans un cas réel. Elle doit être lue avec le contexte de charge, de version et d’architecture. |
| PDU | Cette information indique comment le mécanisme PDU se comporte dans un cas réel. Elle doit être lue avec le contexte de charge, de version et d’architecture. |
| Capteur matériel | Cette information indique comment le mécanisme Capteur matériel se comporte dans un cas réel. Elle doit être lue avec le contexte de charge, de version et d’architecture. |
    ...
    Vous êtes responsable du sujet **Monitoring Other Components** sur une plateforme Exadata de formation. À partir du scénario suivant, rédigez une analyse de deux pages :
    ...
    Une bonne réponse commence par identifier les composants du chapitre : **ILOM, PDU, Capteur matériel**. Elle explique ensuite le chemin technique suivi par l’opération et indique pourquoi les commandes proposées permettent de vérifier ce chemin. Les commandes attendues sont celles de la section 7, adaptées aux noms réels de l’environnement.
    ...
    ```text
    À retenir
    - Monitoring Other Components  : base, cluster, ASM, storage cells, réseau et outils Oracle.
    - Les notions centrales du chapitre sont : ILOM, PDU, Capteur matériel.
...
Cette section rend visible la finition experte V5 pour **Monitoring des composants complémentaires**. Elle impose un raisonnement lié aux objets réels du thème plutôt qu’une formule répétée entre modules.

| Élément expert V5 | Application concrète au module |
|---|---|
| Objets à contrôler | ILOM, PDU, KVM, firmware, environnement, supervision datacenter. |
| Méthode de diagnostic | ne pas ignorer les composants non database lorsqu’ils portent le risque physique. |
| Cas d’école attendu | une alerte alimentation ou température peut précéder une dégradation de service. |
...
ipmitool sensor 2>/dev/null || true
ipmitool sel list 2>/dev/null || true
```
...
Pour les composants physiques annexes, le monitoring expert relie métriques Exadata, vues Oracle, alertes et chronologie. La valeur pédagogique vient de l’interprétation, pas de l’accumulation de sorties brutes.
